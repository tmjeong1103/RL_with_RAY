...
for t in range(int(total_steps)):
    ...
    for _ in range(int(update_count)):
        ...
        val_loss = calc_q_loss(model=R.model,target=R.target,data=replay_buffer)
        val_loss.backward()
        R.train_q_op.step()
        ...
        pi_loss = calc_pi_loss(model=R.model, data=replay_buffer)
        pi_loss.backward()
        R.train_pi_op.step()
        ...
        with torch.no_grad():
            for v_main, v_targ in zip(R.model.parameters(), R.target.parameters()):
                ...
                v_targ.data.mul_(polyak)
                v_targ.data.add_((1 - polyak) * v_main.data) # polyak*v_targ + (1-polyak)*v_main)
